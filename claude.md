...
**Project:** Static recipe flashcard website with JSON-based recipe storage, deployed to GitHub Pages.

**Live Site:** https://github.com/NickOOOShea/recipes (GitHub Pages auto-deploys from master branch)

**Capabilities:**
- 44+ recipes with search, tag filter, and sort (alphabetical / cook time)
- Modular recipes (interactive component selectors) — see `ramen-modular.json`
- Recipe modal with cook mode, scale 0.5x–4x, print button
- Tap-to-start countdown timers parsed from step text ("simmer for 30 min" becomes a clickable button)
- Offline-capable via service worker; installable PWA via `manifest.json`
- Print stylesheet that renders the currently-open recipe in two-column ingredient format
- All recipe content escaped before injection (XSS-safe even if a future imported recipe contains markup)
...
- `steps`: Array of instruction strings. Any phrase matching `\b\d+(\.\d+)?\s*(sec(s|onds)?|min(s|utes)?|hr(s|ours)?)\b` is auto-rendered as a tap-to-start timer button at runtime, so write durations in natural language.
- `notes`: Array of additional comments
- `created_at` / `last_updated_at`: YYYY-MM-DD format

There is no `source_url` field (removed in commit `43eda37`). Don't reintroduce it.

### Bundled Loading
`app.js` fetches a single `recipes/bundle.json` instead of N+1 per-file requests. The bundle is regenerated by `build.ps1`, which reads `recipes/index.json` for the canonical list of files and emits `{ generated_at, count, recipes }`.

**Always run `./build.ps1` after adding, editing, or removing a recipe.** The site will load stale content otherwise.
...
8. **Run `./build.ps1` to regenerate `recipes/bundle.json`** — the site reads this bundle, not the individual files
9. Commit: `git add . && git commit -m "Add recipe: <Title>"`
10. Push: `git push`
11. Show user the final JSON
...
- Steps should be direct and actionable — write durations in natural language ("simmer for 30 min") so the timer parser picks them up
- Use the current date from the session context for `created_at` / `last_updated_at`
- Site is fully static — no backend required
- After changing `app.js`, `styles.css`, or any cached shell asset, bump `CACHE_VERSION` in `sw.js` so returning users pick up the new build
...
├── app.js              # Search, filter, modal, timers, SW registration
├── styles.css          # Purple gradient theme + print + timer panel
├── sw.js               # Service worker — stale-while-revalidate cache
├── manifest.json       # PWA manifest
├── build.ps1           # Regenerates recipes/bundle.json from index.json
├── CLAUDE.md           # This handoff doc
└── recipes/
    ├── index.json      # Canonical list of recipe files (alphabetical)
    ├── bundle.json     # Generated by build.ps1 — what the site actually loads
    └── <slug>.json     # One file per recipe
```
